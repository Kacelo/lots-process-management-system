import {
  ...
  export const ProcessReviewTable = observer(() => {
    ...
    const fetchData = async () => {
        try {
          ...
          // Query for processes with the status "completed"
          const completedProcessesQuery = query(
            processCollection,
            where("status", "==", "completed") // Filter by the "status" field
          );
      
          const querySnapshot = await getDocs(completedProcessesQuery);
    ...
    const handleEditClick = (processId: string) => {
      ...
      router.push(`/dashboard/scrutinize-processes?processId=${processId}`);
    };
    if (isLoading) {
      return (
        <div>
